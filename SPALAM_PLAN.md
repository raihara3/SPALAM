...
- [x] カメラ距離の推定
  - [x] 特徴点のスケール変化から相対距離を計算
  - [x] 複数の安定特徴点間の距離比率で深度を更新
  - [x] IMU加速度計は使用しない（ドリフトが大きすぎるため）
- [x] 平面メッシュのスケール更新
  - [x] 深度変化に応じた平面サイズの動的調整
  - [x] スムージング処理の実装
    - [x] 指数平滑化（Exponential Smoothing）によるスケール値の安定化
    - [x] スムージング係数の調整（応答性と安定性のバランス）

**2.5.2 特徴点の固定化**

- [x] 特徴点ロック機構の実装
  - [x] 初回検出時の特徴点位置を基準として保存（アンカー特徴点）
  - [x] 追跡中の特徴点の位置更新を制限
  - [x] ロックされた特徴点が見えなくなった場合の再取得戦略
- [x] 特徴点の安定性評価強化
  - [x] 追跡品質に基づく特徴点の選別
  - [x] 不安定な特徴点の除外
  - [x] 再投影誤差フィルタリング
    - [x] ロックした3D点を現フレームに投影
    - [x] 実際の観測点との距離（再投影誤差）を計算
    - [x] 閾値を超えた場合は「不安定」として除外または重み低下
- [x] オプティカルフローのドリフト防止
  - [x] Forward-Backward Check（往復誤差チェック）
    - [x] 前→現、現→前の双方向でオプティカルフローを計算
    - [x] 往復誤差が閾値（1.0px）以上の特徴点を除外
  - [x] 動きの整合性チェック
    - [x] 全特徴点の動きベクトルの中央値を計算
    - [x] MAD（中央値絶対偏差）で外れ値を検出・除外
  - [x] 定期的な記述子再照合（N フレームごと）
  - [x] 記述子マッチングによる位置補正
  - [x] 照合失敗時の特徴点再検出
...
- [x] 平面フィッティングの改善
  - [x] より多くの特徴点を使用した平面推定
  - [x] 外れ値除去の強化（RANSAC閾値調整）
  - [x] 時間的なスムージング（複数フレームの平均）
- [x] IMU重力ベクトルとの整合性チェック
  - [x] 水平面の場合、重力方向と法線の関係を利用
  - [x] 角度の微調整
...
class ExponentialSmoother {
  ...
  private alpha: number = 0.1; // 0に近いほど安定、1に近いほど応答性高
...
改善:
1. 初回検出時に「アンカー特徴点」を設定（位置 + 記述子を保存）
2. 以降はオプティカルフロー追跡のみ使用
3. N フレームごとに記述子再照合でドリフト補正
4. 追跡が失敗した場合のみ再検出
```
...
```
1. ロックした3D点 P_world を現フレームのカメラ行列で2Dに投影
   p_projected = K * [R|t] * P_world

2. オプティカルフローで追跡した2D点 p_tracked との距離を計算
   error = ||p_projected - p_tracked||

3. 閾値判定
   - error < 5px: 安定（重み 1.0）
   - 5px <= error < 10px: やや不安定（重み 0.5）
   - error >= 10px: 不安定（除外）
```

#### Phase 2.5 完了基準

- [x] カメラを近づけると平面メッシュが大きく見える
- [x] カメラを離すと平面メッシュが小さく見える
- [x] 特徴点が安定して固定される（フレーム間でジッターしない）
- [x] 平面の角度が実際の面により近い
...
| 日付       | 内容                                                                                     |
| ---------- | ---------------------------------------------------------------------------------------- |
| 2025-12-29 | **Phase 2.5最終完了**: Forward-Backward Check, 動きの整合性チェック, 品質フィルタ強化, 重力補正修正 |
| 2025-12-29 | **Phase 2.5実装完了**: DistanceTracker, FeatureAnchor, GravityAligner, SPALAM統合        |
| 2025-12-29 | **Phase 2.5計画強化**: ADVICE.mdから再投影誤差フィルタリング、記述子再照合、スムージング手法を追加 |
