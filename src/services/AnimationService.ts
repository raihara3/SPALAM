...
export class AnimationService {
  ...
  public addCustomAnimation(
    ...
    // キーフレームを時間順にソート
    const sortedKeyframes = [...animationConfig.keyframes].sort((a, b) => a.time - b.time);
    ...
    ['x', 'y', 'z'].forEach(axis => {
      ...
      if (axisTimes.length > 0) {
        ...
        tracks.push(new THREE.NumberKeyframeTrack(trackName, axisTimes, axisValues));
      }
    });
    
    if (tracks.length > 0) {
      const clip = new THREE.AnimationClip(animationConfig.name, animationConfig.duration, tracks);
  ...
  private updateFloat(animation: Animation): void {
    const { amplitude, frequency, initialY } = animation.config;
    const time = animation.currentTime;
    
    animation.object.position.y = initialY + Math.sin(time * frequency) * amplitude;
  }
  ...
  private updatePulse(animation: Animation): void {
    const { minScale, maxScale, speed, initialScale } = animation.config;
    const time = animation.currentTime;
    
    const scale = minScale + (maxScale - minScale) * (Math.sin(time * speed) * 0.5 + 0.5);
    animation.object.scale.copy(initialScale).multiplyScalar(scale);
  }
  ...
  public stop(id: string): void {
    ...
    if (animation) {
      ...
      switch (animation.type) {
        case "float":
          animation.object.position.y = animation.config.initialY;
          break;
        case "pulse":
          animation.object.scale.copy(animation.config.initialScale);
          break;
      }
    }
